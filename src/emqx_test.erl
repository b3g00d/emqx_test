...
-define(LOG(Level, Format, Args), emqx_logger:Level("emqx_test: " ++ Format, Args)).

register_metrics() ->
    [emqx_metrics:new(MetricName) || MetricName <- ['emqx_test.client_connected',
                                                    'emqx_test.client_disconnected',
                                                    'emqx_test.client_subscribe',
                                                    'emqx_test.client_unsubscribe',
                                                    'emqx_test.session_subscribed',
                                                    'emqx_test.session_unsubscribed',
                                                    'emqx_test.message_publish',
                                                    'emqx_test.message_delivered',
                                                    'emqx_test.message_acked']].
...
on_client_connected(#{clientid := ClientId, username := Username}, ConnInfo, _Env) ->
    emqx_metrics:inc('emqx_test.client_connected'),
    %% Code Start
...
on_client_disconnected(#{clientid := ClientId, username := Username}, Reason, _ConnInfo, _Env)
    when is_atom(Reason) ->
    emqx_metrics:inc('emqx_test.client_disconnected'),
    %% Code Start
...
on_client_subscribe(#{clientid := ClientId, username := Username}, Properties, RawTopicFilters, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('emqx_test.client_subscribe'),
          %% Code Start
...
on_client_unsubscribe(#{clientid := ClientId, username := Username}, Properties, RawTopicFilters, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('emqx_test.client_unsubscribe'),
          %% Code Start
...
on_session_subscribed(#{clientid := ClientId}, Topic, SubOpts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('emqx_test.session_subscribed'),
        %% Code Start
...
on_session_unsubscribed(#{clientid := ClientId}, Topic, Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('emqx_test.session_unsubscribed'),
        %% Code Start
...
on_message_publish(Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('emqx_test.message_publish'),
        %% Code Start
...
on_message_delivered(#{clientid := ClientId, username := Username},
  ...
  with_filter(
    fun() ->
      emqx_metrics:inc('emqx_test.message_delivered'),
      %% Code Start
...
on_message_acked(#{clientid := ClientId}, Message = #message{topic = Topic}, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('emqx_test.message_acked'),
        %% Code Start
